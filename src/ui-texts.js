export default {
  en: {},
  et: {
    chat: { innerHtml: 'Tegijate jututuba' },
    docs: { innerHtml: '<b>Vue.js</b> koduleht' },
    eco: { innerHtml: 'Kooslus' },
    emulate: { innerHtml: 'Emuleeri seisundimuutust' },
    forum: { innerHtml: 'Foorum' },
    lang: { innerHtml: 'English (EN)' },
    links: { innerHtml: 'Olulised lingid' },
    news: { innerHtml: 'Uudised' },
    plugins: { innerHtml: 'Installitud CLI pluginad' },
    trace: {innerHtml: 'Näita trasseeringut<br>debugeri aknas'},
    try: {innerHtml: 'Proovi, mis teeb'},
    'For a detailed description of how this works, check out the':
      'Kuidas see kõik töötab, seda selgitab',
    'vue-proper documentation': 'vue-proper\'i dokumentatsioon',
    'Welcome to vue-proper demo App!': 'Tere tulemast vue-proper demorakendusse!'
  }
}
